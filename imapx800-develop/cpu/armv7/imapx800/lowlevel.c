...
void module_enable(uint32_t base) {
	...
	/* wait power on ACK */
//	while(!(readl(base + 8) & 2));

	/* enable module output */
	writel(readl(base + 8) & ~0x10, base + 8);
...
void reset_core(int index, uint32_t hold_base)
{
	...
	writel((hold_base >> 8) & 0xff, RTC_INFO1);
	writel((hold_base >> 16) & 0xff, RTC_INFO2);
	writel((hold_base >> 24) & 0xff, RTC_INFO3);
	writel(0x3b, RTC_INFO0);
