...
int burn_extra(struct ius_desc *desc, int id_i)
{
	...
	if(id_o == DEV_NAND) {
		...
		if(ius_remain % 2064) {
			ret = -3;                          
			printf("Here only accept 2K+16 page data.\n");
			goto __out;                        

		}                                      

		/* this will be treat as a yaffs image */
		ret = yaffs_burn(offs, size, ius_get_page);
	} else {
		/* this will be treat as a sparse image */
		ret = sparse_burn(id_o, offs, ius_remain, step,
				ius_sparse_data);
	}
