...
//					id[0], id[1], id[2], id[3], id[4], id[5], id[6], id[7]);
	...
	gp_retrylevel = (uint32_t *)(CONFIG_RESERVED_RRTB_BUFFER + 0x4);
	...
	if(nand_cfg.normal.read_retry == 1){
		if(nand_cfg.normal.nand_param0 == 0x55){
			if(nand_cfg.normal.nand_param1 == 20){
				...
				g_retrylevel = *((uint32_t *)(CONFIG_RESERVED_RRTB_BUFFER + 0x4));
				retry_param = (uint8_t *)(CONFIG_RESERVED_RRTB_BUFFER +0x100);
				retry_param_magic = *((uint32_t *)(CONFIG_RESERVED_RRTB_BUFFER + 0x8));
...
otp_check_again:				
			...
			if(nand_cfg.normal.nand_param1 == 26){
				...
				retry_param = (uint8_t *)(CONFIG_RESERVED_RRTB_BUFFER);
				retry_param_magic = *((uint32_t *)(CONFIG_RESERVED_RRTB_BUFFER + 0x8));
				...
				if((retry_param_magic != 0x8a7a6a5a) || (g_otp_buf[0] == 0x0)){				
					...
					*((uint32_t *)(CONFIG_RESERVED_RRTB_BUFFER + 0x8)) = 0x8a7a6a5a;
...
#if 0
				g_otp_buf[0] = readl(RTC_INFO1);
				g_otp_buf[1] = readl(RTC_INFO2);
				g_otp_buf[2] = readl(RTC_INFO3);
				g_otp_buf[3] = readl(RTC_INFO5);
				g_retrylevel = readl(RTC_INFO0);
				*(retry_param + 0) = g_otp_buf[0];
				*(retry_param + 1) = g_otp_buf[1];
				*(retry_param + 2) = g_otp_buf[2];
				*(retry_param + 3) = g_otp_buf[3];
				*((uint32_t *)88000004) = g_retrylevel;
#endif
